...
function ProfileSubTask({ subTask, currentUser, owned }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/users/${currentUser.id}/completedsubtasks`)
      .then((resp) => resp.json())
      .then((data) => {
        setUsersCompletedSubTasks(data);
  ...
  function handleCompleteTask() {
    ...
    fetch(`http://localhost:3000/completed_subtasks`, {
      ...
      .then((resp) => resp.json())
      .then((data) =>
        setUsersCompletedSubTasks([...usersCompletedSubTasks, data])
      )
  ...
  function handleUndoTask() {
    console.log("undo");
    const thisTask = usersCompletedSubTasks.filter(
      (t) => t.sub_task_id == subTask.id
    );
  ...
  return (
    <div className="profileSubTask">
      {editMode ? undefined : completed ? (
        <input type={"checkbox"} checked onClick={() => handleUndoTask()} />
      ) : (
