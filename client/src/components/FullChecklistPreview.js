...
function FullChecklistPreview({ setCurrentUser, currentUser }) {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/checklists/${id}`)
      ...
      .then((data) => {
        ...
        setRenderChecklist(data.tasks.map((t) => <TaskPreview task={t} />));
  ...
  function handleUnfollow() {
    ...
    const targetedUnfollow = usersFollows.find(
      (f) =>
        f.checklist_id == currentChecklist.id && f.user_id == currentUser.id
    ).id;
        ...
        <div>
          ...
          <div className="FullChecklistPreview">
            ...
            {loaded ? (
              currentUser ? (
                currentUser.id ==
                currentChecklist.user_id ? undefined : followsRendered.find(
            ...
            <div className="previewChecklist">{renderChecklist}</div>
          </div>
        </div>
      ) : undefined}
    </div>
